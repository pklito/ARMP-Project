...
T_AB = np.eye(4) # transition matrix from frame A (UR3E) to frame B (UR5E)
T_AB[0, 3] = -0.13  # Distance along the x-axis from frame A to frame B
T_AB[1, 3] = -0.37  # Distance along the y-axis from frame A to frame B
T_AB[2, 3] = 0.00  # Distance along the z-axis from frame A to frame B
...
def quat_to_ur_axis_angle(quaternion):
    ...
    angle = 2*atan2(np.linalg.norm(quaternion[0], quaternion[1], quaternion[2]), quaternion[3])
    if abs(angle) > 1e-6:
...
def calculate_assistant_robot_path(ur3e_arm, ur5e_arm, task_robot_path):
    ...
    bb_ur5e = Building_Blocks_UR5e(transform=transform_ur5e, ur_params=ur_params_ur5e, env=None, resolution=0.1, p_bias=0.05)
    bb_ur3e = Building_Blocks_UR3e(transform=transform_ur3e, ur_params=ur_params_ur3e, env=None, resolution=0.1, p_bias=0.05) # should implement seperate collision functions

    assistant_path = []
    # calculate FK to get the position of the end effector on the UR3E
    for index, conf in enumerate(task_robot_path):
        quaternion_ur3e = ur3e_arm.forward(conf)
        xyz_position_ur3e = quaternion_ur3e[:3]

        # Transition to UR5E frame
        position_in_ur5e_frame = transition_coordinates(xyz_position_ur3e, T_AB)
        ...
        try:
            ...
            if ur5e_joint_angles is not None:
                assistant_path.append(ur5e_arm)
...
# task_robot_path = [np.array([-0.17453293, -1.3962634 ,  0.17453293, -1.22173048,  0.17453293,
...
#     0.        ])]
# calculate_camera_robot_transitions(ur3e_arm, ur5e_arm, task_robot_path)
